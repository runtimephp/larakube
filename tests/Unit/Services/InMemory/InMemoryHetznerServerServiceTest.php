...
beforeEach(function (): void {
    /** @var InMemoryHetznerServerService $this->service */
...
test('destroy marks server as deleted but getAll excludes it', function (): void {
    /** @var ServerData $server */
    $server = new ServerData(
        externalId: '123',
        name: 'web-1',
...
test('find returns null for deleted server', function (): void {
    /** @var ServerData $server */
    $server = new ServerData(
        externalId: '123',
        name: 'web-1',
    ...
    expect($this->service->find('web-1'))->toBeNull();
});
...
test('add server returns self for fluent interface', function (): void {
    ...
    expect($this->service->addServer($server))->toBeInstanceOf(InMemoryHetznerServerService::class);
});

test('shouldFailCreate returns self for fluent interface', function (): void {
    expect($this->service->shouldFailCreate(true))->toBeInstanceOf(InMemoryHetznerServerService::class);
});

test('shouldFailDelete returns self for fluent interface', function (): void {
    expect($this->service->shouldFailDelete(true))->toBeInstanceOf(InMemoryHetznerServerService::class);
});
